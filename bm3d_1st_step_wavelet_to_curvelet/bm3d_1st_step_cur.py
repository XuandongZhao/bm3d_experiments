...
from curvelet_2d import curvelet_2d_forward, curvelet_2d_reverse
...
def bm3d_1st_step_cur(sigma, img_noisy, nHard, kHard, NHard, pHard, lambdaHard3D, tauMatch, useSD, tau_2D):
    ...
    elif tau_2D == 'BIOT':  # 'BIOR'
        fre_all_patches = bior_2d_forward(all_patches)
    elif tau_2D == 'CUR':
        fre_all_patches = curvelet_2d_forward(all_patches)
        # fre_all_patches = pc.fdct(all_patches)
    ...
    if tau_2D == 'DCT':
        ...
        group_3D_table = curvelet_2d_reverse(group_3D_table)
